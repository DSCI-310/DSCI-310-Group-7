...
class TestParaOptimize(unittest.TestCase):
    ...
    def test_para_optimize_type(self):
        self.assertTrue(isinstance(para_optimize(knn, param_grid_knn, 2, X, y), dict))
        self.assertTrue(isinstance(para_optimize(dt, param_grid_dt, 2, X, y), dict))
        self.assertTrue(isinstance(para_optimize(svm, param_grid_svm, 2, X, y), dict))
        self.assertTrue(isinstance(para_optimize(lr, param_grid_lr, 2, X, y), dict))
    ...
    def test_para_optimize_modtype(self):
        self.assertEqual(para_optimize("model", param_grid_knn, 4, X, y), "The model is invalid.")
